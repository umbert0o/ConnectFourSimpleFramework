...
P1_PARAMS=("iterations=1000" "exploration_weight=1")
P2_AI="connect_four.ai.mcts_ai.MCTSAI"
P2_BASE_PARAMS=("iterations=1000")
WORKERS=10
GAMES=1000
...
# Loop 0 to 20 → 0.0 to 2.0 in 0.1 increments (31 iterations)
for i in $(seq 0 20); do
    EXPL_WEIGHT=$(awk "BEGIN {printf \"%.1f\", $i/10}")
    OUTPUT_FILE="${OUTPUT_DIR}/MCTS1_1000-1_vs_MCTS2_1000-${EXPL_WEIGHT}.json"

    # Build P2 params array to match argparse nargs="*" expectations [[citation:2]]
    P2_PARAMS=("${P2_BASE_PARAMS[@]}" "exploration_weight=${EXPL_WEIGHT}")

    echo "[$((i+1))/21] P2 exploration_weight=${EXPL_WEIGHT} → ${OUTPUT_FILE}"
